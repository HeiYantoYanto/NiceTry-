...
function buildSiteThemes() {
    ...
    THEMES.forEach(theme => {
        ...
        chip.className = 'customization-option';
        ...
        if (effectiveCurrent === theme.id) chip.classList.add('selected');
...
function buildFrames(flags) {
    ...
    const unlockedFrames = customizations.frame || [];
    ...
    const selected = selectedRaw === 'gold' ? 'default'
        ...
        : selectedRaw;
    FRAMES.forEach(f => {
        // Leave all frames unlocked for testing
        let unlocked = true;
        const chip = document.createElement('button');
...
function buildNavbarThemes() {
    ...
    const NAVBAR_THEMES = [
        ...
        { id: 'purple', bg: 'linear-gradient(90deg, #6B53DE, #9B7BFF)', label: 'Purple' },
    ...
    // Coerce any previously saved 'light' navbar theme to 'dark'
    if (prefs.navbarTheme && prefs.navbarTheme.id === 'light') {
        savePref('navbarTheme', { id: 'dark', label: 'Dark', bg: 'var(--color-bg-deep)' });
    }
    
    const email = sessionStorage.getItem('currentUser');
    const customizations = email ? JSON.parse(localStorage.getItem(`customizations_${email}`)) || {} : {};
    const unlockedThemes = customizations.navbarTheme || [];
    
    NAVBAR_THEMES.forEach(theme => {
        // Temporarily leave all navbar themes unlocked
        const unlocked = true;
        const chip = document.createElement('button');
        chip.className = 'customization-option' + (unlocked ? '' : ' locked');
...
function buildSpecialEffects() {
    ...
    const email = sessionStorage.getItem('currentUser');
    const customizations = email ? JSON.parse(localStorage.getItem(`customizations_${email}`)) || {} : {};
    const unlockedEffects = customizations.specialEffect || [];
    ...
    SPECIAL_EFFECTS.forEach(effect => {
        // Temporarily leave all special effects unlocked
        const unlocked = true;
        const chip = document.createElement('button');
        chip.className = 'customization-option' + (unlocked ? '' : ' locked');
