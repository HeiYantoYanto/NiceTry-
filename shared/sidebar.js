...
function showSidebar() {
    ...
    if (settingsBtn) {
        settingsBtn.addEventListener('click', function(e) {
            e.preventDefault();
            hideSidebar();
            const path = getCorrectPath('profile/settings.html');
...
function getCorrectPath(relativePath) {
    ...
    if (path.includes('/games/')) return '../' + relativePath;
    if (path.includes('/login_signup/')) return '../../' + relativePath;
    if (path.includes('/team_info/')) return '../../' + relativePath;
    if (path.includes('/course/')) return '../../../' + relativePath;
    if (path.includes('/profile/')) return '../' + relativePath; // navigating from inside /profile back to /profile targets
    return relativePath;
}
